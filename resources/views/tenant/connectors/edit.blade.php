<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-gray-900 leading-tight">Edit Connector</h2>
    ...
    $activeMeta   = $connectorMeta[$cType];
    $activeColors = $typeColors[$activeMeta['color']];
    $typeFields   = \App\Models\Connector::TYPE_FIELDS[$cType];
    $fileTypes    = \App\Models\Connector::FILE_TYPES;
    $savedTypes   = $settings['file_types'] ?? \App\Models\Connector::DEFAULT_FILE_TYPES;
    @endphp
            ...
            async runTest() {
                ...
                try {
                    const res = await fetch('{{ route('tenant.connectors.test', ['tenantSlug' => $tenant->slug]) }}', {
                        ...
                        body: JSON.stringify({ type: '{{ $cType }}', settings }),
                    });
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-2 space-y-1">
                            ...
                            <div class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium
                                {{ $cType === $t ? 'bg-[#0f62fe] text-white shadow-sm' : 'text-gray-400' }}">
                                <svg class="w-4 h-4 shrink-0 {{ $cType === $t ? 'text-white' : $c['text'] }}"
                    ...
                    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                        ...
                        <div class="p-2 space-y-0.5">
                            ...
                            <form method="POST"
                                  ...
                                  onsubmit="return confirm('Delete this connector? This cannot be undone.')">
                                @csrf @method('DELETE')
                                <button type="submit"
                ...
                <div class="flex-1 min-w-0">
                    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                        ...
                        <div class="lg:col-span-3 space-y-5">
                            ...
                            {{-- Type banner --}}
                            <div class="bg-[#001141] rounded-xl px-6 py-4 flex items-center justify-between gap-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="text-white font-semibold text-sm">{{ $connector->name }}</p>
                                        <p class="text-blue-200 text-xs mt-0.5">{{ $activeMeta['label'] }} Connector</p>
                                    </div>
                                </div>
                                <span class="text-[10px] font-bold px-2.5 py-1 rounded-full
                                    {{ $connector->status === 'active' ? 'bg-green-400/20 text-green-300' : 'bg-gray-400/20 text-gray-300' }}">
                                    {{ ucfirst($connector->status) }}
                                </span>
                            </div>

                            <form id="connector-form" method="POST"
                                  action="{{ route('tenant.connectors.update', ['tenantSlug' => $tenant->slug, 'connector' => $connector->id]) }}"
                                  class="space-y-5">
                                @csrf @method('PATCH')

                                {{-- SECTION 1: Display Name & Status --}}
                                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                    <div class="px-5 py-3 border-b border-gray-100 bg-gray-50">
                                        <p class="text-xs font-bold text-gray-600 uppercase tracking-widest">Display Name & Status</p>
                                    </div>
                                    <div class="px-5 py-5 space-y-4">
                                        <div>
                                            <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Display Name</label>
                                            <x-text-input id="name" name="name" type="text" class="block w-full"
                                                value="{{ old('name', $connector->name) }}" required autofocus />
                                ...
                                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                                    <div class="px-5 py-3 border-b border-gray-100 bg-gray-50">
                                        ...
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $activeMeta['label'] }} connection settings.</p>
                                    </div>
                                        ...
                                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            ...
                                            <label class="flex items-center gap-2.5 cursor-pointer group">
                                                <input type="checkbox" name="file_types[]" value="{{ $key }}"
                                                    {{ in_array($key, old('file_types', $savedTypes)) ? 'checked' : '' }}
