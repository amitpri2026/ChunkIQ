        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @php
                ...
                $connStatusBadge = $connectorCount > 0
                    ? ['label' => 'Active',    'class' => 'bg-teal-100 text-teal-700']
                ...
                $cfgBadge = $configSteps === 3
                    ? ['label' => 'All done',            'class' => 'bg-green-100 text-green-700']
                    : ($configSteps > 0
                        ? ['label' => "{$configSteps}/3 done", 'class' => 'bg-amber-100 text-amber-700']
                        : ['label' => 'Pending',              'class' => 'bg-red-100 text-red-600']);
            @endphp

            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h2 class="font-bold text-gray-700 mb-5">Status</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">

                    {{-- Connectors --}}
                    <div class="flex flex-col p-5 rounded-xl border-2 border-teal-100 bg-gradient-to-br from-teal-50 to-white">
                        <div class="flex items-start justify-between mb-3">
                            <div class="w-10 h-10 rounded-lg bg-teal-100 flex items-center justify-center text-teal-600 flex-shrink-0">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>
                                </svg>
                            </div>
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $connStatusBadge['class'] }}">
                                {{ $connStatusBadge['label'] }}
                            </span>
                        </div>
                        <p class="text-3xl font-extrabold text-gray-800">{{ $connectorCount }}</p>
                        <p class="text-xs font-semibold text-gray-500 mt-1">Connectors</p>
                        <p class="text-xs text-gray-400 mt-0.5">SharePoint · Teams · OneDrive</p>
                    </div>

                    {{-- Pipeline Jobs --}}
                    <div class="flex flex-col p-5 rounded-xl border-2 border-orange-100 bg-gradient-to-br from-orange-50 to-white">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $jobStatusBadge['class'] }}">
                                {{ $jobStatusBadge['label'] }}
                            </span>
                        </div>
                        <p class="text-3xl font-extrabold text-gray-800">{{ $jobCount }}</p>
                        <p class="text-xs font-semibold text-gray-500 mt-1">Pipeline Jobs</p>
                        <p class="text-xs text-gray-400 mt-0.5">Ingestion &amp; processing</p>
                    </div>

                    {{-- Members --}}
                    <div class="flex flex-col p-5 rounded-xl border-2 border-purple-100 bg-gradient-to-br from-purple-50 to-white">
                        <div class="flex items-start justify-between mb-3">
                        ...
                        <p class="text-3xl font-extrabold text-gray-800">{{ $memberCount }}</p>
                        <p class="text-xs font-semibold text-gray-500 mt-1">Members</p>
                        <p class="text-xs text-gray-400 mt-0.5">Admins &amp; users</p>
                    </div>

                    {{-- Azure Config --}}
                    <div class="flex flex-col p-5 rounded-xl border-2 border-blue-100 bg-gradient-to-br from-blue-50 to-white">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $cfgBadge['class'] }}">
                                {{ $cfgBadge['label'] }}
                            </span>
                        </div>
                        <p class="text-3xl font-extrabold {{ $configSteps === 3 ? 'text-green-600' : ($configSteps > 0 ? 'text-amber-500' : 'text-red-500') }}">
                            {{ $configSteps }}/3
                        </p>
                        <p class="text-xs font-semibold text-gray-500 mt-1">Azure Config</p>
                        <p class="text-xs text-gray-400 mt-0.5">Steps completed</p>
                    </div>
